  ...
  return (
    ...
    >
      <Link to={`/products?category=${category.id}`} className="block">
        {/* Icon/Image Section */}
        <div className={`relative h-32 sm:h-40 md:h-48 flex items-center justify-center ${category.icon && category.icon.startsWith('http') ? 'bg-slate-800' : `bg-gradient-to-br ${gradient}`} overflow-hidden`}>
          {category.icon && category.icon.startsWith('http') ? (
            ...
            <motion.div
              ...
              className="text-4xl sm:text-6xl md:text-8xl filter drop-shadow-2xl"
            >
              {category.icon}
            </motion.div>
          )}
          ...
          {/* Count Badge */}
          <div className="absolute top-2 right-2 sm:top-4 sm:right-4 bg-black/50 backdrop-blur-sm rounded-full px-2 py-1 sm:px-3">
        ...
        <div className="p-3 sm:p-4 md:p-6">
          <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-theme-heading mb-2 group-hover:text-gradient transition-all line-clamp-2">
            {category.name}
          </h3>
          <p className="text-theme-secondary group-hover:text-theme transition-colors text-sm sm:text-base line-clamp-2">
            {category.description}
          </p>
          
          <div className="mt-3 sm:mt-4 flex items-center text-theme-heading font-semibold group-hover:translate-x-2 transition-transform text-sm sm:text-base">
